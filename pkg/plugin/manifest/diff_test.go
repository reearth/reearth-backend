...
func TestDiffFrom(t *testing.T) {
	...
	oldps := id.MustPropertySchemaID("aaaaaa~1.0.0/_")
	...
	old := Manifest{
		...
		ExtensionSchema: []*property.Schema{
			...
			property.NewSchema().ID(olde5ps).Groups([]*property.SchemaGroup{
	...
	new := Manifest{
		...
		ExtensionSchema: []*property.Schema{
			...
			property.NewSchema().ID(olde3ps).MustBuild(),
			property.NewSchema().ID(olde4ps).Groups([]*property.SchemaGroup{
