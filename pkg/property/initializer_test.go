...
func TestInitializer_Property(t *testing.T) {
	...
	expected := New().ID(*initializer.ID).Schema(initializer.Schema).Scene(sid).Items([]Item{
		NewItem().ID(*initializer.Items[0].ID).Schema(initializer.Items[0].SchemaItem).Group().MustBuild(),
	}).MustBuild()
...
func TestInitializerItem_PropertyItem(t *testing.T) {
	...
	expected := NewItem().ID(*item.ID).Schema(item.SchemaItem).Group().MustBuild()
...
func TestInitializerItem_PropertyGroup(t *testing.T) {
	...
	expected := NewItem().ID(*item.ID).Schema(item.SchemaItem).Group().Fields([]*Field{
...
func TestInitializerItem_PropertyGroupList(t *testing.T) {
	...
	expected := NewItem().ID(*item.ID).Schema(item.SchemaItem).GroupList().Groups([]*Group{
		NewItem().ID(*item.Groups[0].ID).Schema(item.SchemaItem).Group().MustBuild(),
	}).MustBuild()
...
func TestInitializerGroup_PropertyGroup(t *testing.T) {
	...
	expected := NewItem().ID(*item.ID).Schema(parentItem).Group().Fields([]*Field{
		NewField().Field(item.Fields[0].Field).Value(NewOptionalValue(item.Fields[0].Type, item.Fields[0].Value)).Build(),
	}).MustBuild()
