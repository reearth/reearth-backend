package mongo

import (
	"context"
	"errors"

	"github.com/reearth/reearth-backend/internal/infrastructure/mongo/mongodoc"
	"github.com/reearth/reearth-backend/internal/usecase/repo"
	"github.com/reearth/reearth-backend/pkg/config"
	"github.com/reearth/reearth-backend/pkg/rerror"
	"go.mongodb.org/mongo-driver/bson"
	"go.mongodb.org/mongo-driver/mongo"
	"go.mongodb.org/mongo-driver/mongo/options"
)

var upsert = true

type configRepo struct {
	client *mongodoc.ClientCollection
}

func NewConfig(client *mongodoc.Client) repo.Config {
	return &configRepo{client: client.WithCollection("config")}
}

func (r *configRepo) Load(ctx context.Context) (*config.Config, error) {
	cfg := &config.Config{}
	if err := r.client.Collection().FindOne(ctx, bson.D{}).Decode(cfg); err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			return cfg, nil
		}
		return nil, rerror.ErrInternalBy(err)
	}
	return cfg, nil
}

func (r *configRepo) Save(ctx context.Context, cfg *config.Config) error {
	if cfg == nil {
		return nil
	}
	if _, err := r.client.Collection().UpdateOne(ctx, bson.D{}, bson.M{"$set": cfg}, &options.UpdateOptions{
		Upsert: &upsert,
	}); err != nil {
		return rerror.ErrInternalBy(err)
	}
	return nil
}
