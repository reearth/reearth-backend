...
func TestTag(t *testing.T) {
	...
	t3, _ := tag.NewItem().NewID().Scene(sid).Label("item2").Build()
	tti2 := tag.Tag(t3)
	ttg := tag.Tag(t2)
	tti := tag.Tag(t1)
	...
	out3, err := repo.FindByScene(ctx, sid)
	assert.NoError(t, err)
	assert.Equal(t, []*tag.Tag{&tti, &ttg, &tti2}, out3)
	...
	assert.Nil(t, out9)
	_ = repo.RemoveByScene(ctx, sid)
	out10, _ := repo.FindByID(ctx, t3.ID(), sl)
	assert.Nil(t, out10)
