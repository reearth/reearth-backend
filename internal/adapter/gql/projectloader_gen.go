...
type ProjectLoader struct {
	...
	cache map[id.ProjectID]*Project
...
func (l *ProjectLoader) Load(key id.ProjectID) (*Project, error) {
...
func (l *ProjectLoader) LoadThunk(key id.ProjectID) func() (*Project, error) {
	l.mu.Lock()
...
func (l *ProjectLoader) LoadAll(keys []id.ProjectID) ([]*Project, []error) {
	results := make([]func() (*Project, error), len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ProjectLoader) LoadAllThunk(keys []id.ProjectID) func() ([]*Project, []error) {
	results := make([]func() (*Project, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ProjectLoader) Prime(key id.ProjectID, value *Project) bool {
	l.mu.Lock()
	var found bool
...
func (l *ProjectLoader) unsafeSet(key id.ProjectID, value *Project) {
	if l.cache == nil {
		l.cache = map[id.ProjectID]*Project{}
	}
	l.cache[key] = value
}
