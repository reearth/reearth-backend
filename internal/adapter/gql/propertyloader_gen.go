...
type PropertyLoader struct {
	...
	cache map[id.PropertyID]*Property
...
func (l *PropertyLoader) Load(key id.PropertyID) (*Property, error) {
...
func (l *PropertyLoader) LoadThunk(key id.PropertyID) func() (*Property, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*Property, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *PropertyLoader) LoadAll(keys []id.PropertyID) ([]*Property, []error) {
	results := make([]func() (*Property, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	propertys := make([]*Property, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *PropertyLoader) LoadAllThunk(keys []id.PropertyID) func() ([]*Property, []error) {
	results := make([]func() (*Property, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *PropertyLoader) Prime(key id.PropertyID, value *Property) bool {
	l.mu.Lock()
	var found bool
...
func (l *PropertyLoader) unsafeSet(key id.PropertyID, value *Property) {
	if l.cache == nil {
		l.cache = map[id.PropertyID]*Property{}
	}
	l.cache[key] = value
}
