...
func (l *TeamLoader) Load(key id.TeamID) (*Team, error) {
...
func (l *TeamLoader) LoadThunk(key id.TeamID) func() (*Team, error) {
	l.mu.Lock()
...
func (l *TeamLoader) LoadAll(keys []id.TeamID) ([]*Team, []error) {
	results := make([]func() (*Team, error), len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TeamLoader) LoadAllThunk(keys []id.TeamID) func() ([]*Team, []error) {
	results := make([]func() (*Team, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TeamLoader) Prime(key id.TeamID, value *Team) bool {
	l.mu.Lock()
	var found bool
...
func (l *TeamLoader) unsafeSet(key id.TeamID, value *Team) {
	if l.cache == nil {
		l.cache = map[id.TeamID]*Team{}
	}
	l.cache[key] = value
}
