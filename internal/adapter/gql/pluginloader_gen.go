...
func (l *PluginLoader) Load(key id.PluginID) (*Plugin, error) {
...
func (l *PluginLoader) LoadThunk(key id.PluginID) func() (*Plugin, error) {
	l.mu.Lock()
...
func (l *PluginLoader) LoadAll(keys []id.PluginID) ([]*Plugin, []error) {
	results := make([]func() (*Plugin, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	plugins := make([]*Plugin, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *PluginLoader) LoadAllThunk(keys []id.PluginID) func() ([]*Plugin, []error) {
	results := make([]func() (*Plugin, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *PluginLoader) Prime(key id.PluginID, value *Plugin) bool {
	l.mu.Lock()
	var found bool
...
func (l *PluginLoader) unsafeSet(key id.PluginID, value *Plugin) {
	if l.cache == nil {
		l.cache = map[id.PluginID]*Plugin{}
	}
	l.cache[key] = value
}
