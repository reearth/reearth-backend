...
type LayerGroupLoader struct {
	...
	cache map[id.LayerID]*LayerGroup
...
func (l *LayerGroupLoader) Load(key id.LayerID) (*LayerGroup, error) {
...
func (l *LayerGroupLoader) LoadThunk(key id.LayerID) func() (*LayerGroup, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*LayerGroup, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *LayerGroupLoader) LoadAll(keys []id.LayerID) ([]*LayerGroup, []error) {
	results := make([]func() (*LayerGroup, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	layerGroups := make([]*LayerGroup, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *LayerGroupLoader) LoadAllThunk(keys []id.LayerID) func() ([]*LayerGroup, []error) {
	results := make([]func() (*LayerGroup, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *LayerGroupLoader) Prime(key id.LayerID, value *LayerGroup) bool {
	l.mu.Lock()
	var found bool
...
func (l *LayerGroupLoader) unsafeSet(key id.LayerID, value *LayerGroup) {
	if l.cache == nil {
		l.cache = map[id.LayerID]*LayerGroup{}
	}
	l.cache[key] = value
}
