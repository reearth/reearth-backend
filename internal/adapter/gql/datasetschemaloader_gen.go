...
type DatasetSchemaLoader struct {
	...
	cache map[id.DatasetSchemaID]*DatasetSchema
...
func (l *DatasetSchemaLoader) Load(key id.DatasetSchemaID) (*DatasetSchema, error) {
...
func (l *DatasetSchemaLoader) LoadThunk(key id.DatasetSchemaID) func() (*DatasetSchema, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*DatasetSchema, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *DatasetSchemaLoader) LoadAll(keys []id.DatasetSchemaID) ([]*DatasetSchema, []error) {
	results := make([]func() (*DatasetSchema, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	datasetSchemas := make([]*DatasetSchema, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *DatasetSchemaLoader) LoadAllThunk(keys []id.DatasetSchemaID) func() ([]*DatasetSchema, []error) {
	results := make([]func() (*DatasetSchema, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *DatasetSchemaLoader) Prime(key id.DatasetSchemaID, value *DatasetSchema) bool {
	l.mu.Lock()
	var found bool
...
func (l *DatasetSchemaLoader) unsafeSet(key id.DatasetSchemaID, value *DatasetSchema) {
	if l.cache == nil {
		l.cache = map[id.DatasetSchemaID]*DatasetSchema{}
	}
	l.cache[key] = value
}
...
func (b *datasetSchemaLoaderBatch) keyIndex(l *DatasetSchemaLoader, key id.DatasetSchemaID) int {
