...
func (c *TagLoader) FetchGroupByScene(ctx context.Context, sid id.SceneID) ([]*gqlmodel.TagGroup, error) {
	res, err := c.usecase.FetchGroupsByScene(ctx, sid, getOperator(ctx))
	if err != nil {
		return nil, err
	}
...
func (c *TagLoader) FetchItemByScene(ctx context.Context, sid id.SceneID) ([]*gqlmodel.TagItem, error) {
	res, err := c.usecase.FetchItemsByScene(ctx, sid, getOperator(ctx))
	if err != nil {
		return nil, err
	}
