...
type DatasetLoader struct {
	...
	cache map[id.DatasetID]*Dataset
...
func (l *DatasetLoader) Load(key id.DatasetID) (*Dataset, error) {
...
func (l *DatasetLoader) LoadThunk(key id.DatasetID) func() (*Dataset, error) {
	l.mu.Lock()
...
func (l *DatasetLoader) LoadAll(keys []id.DatasetID) ([]*Dataset, []error) {
	results := make([]func() (*Dataset, error), len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *DatasetLoader) LoadAllThunk(keys []id.DatasetID) func() ([]*Dataset, []error) {
	results := make([]func() (*Dataset, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *DatasetLoader) Prime(key id.DatasetID, value *Dataset) bool {
	l.mu.Lock()
	var found bool
...
func (l *DatasetLoader) unsafeSet(key id.DatasetID, value *Dataset) {
	if l.cache == nil {
		l.cache = map[id.DatasetID]*Dataset{}
	}
	l.cache[key] = value
}
