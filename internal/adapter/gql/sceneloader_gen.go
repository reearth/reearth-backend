...
func (l *SceneLoader) Load(key id.SceneID) (*Scene, error) {
...
func (l *SceneLoader) LoadThunk(key id.SceneID) func() (*Scene, error) {
	l.mu.Lock()
...
func (l *SceneLoader) LoadAll(keys []id.SceneID) ([]*Scene, []error) {
	results := make([]func() (*Scene, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	scenes := make([]*Scene, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *SceneLoader) LoadAllThunk(keys []id.SceneID) func() ([]*Scene, []error) {
	results := make([]func() (*Scene, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *SceneLoader) Prime(key id.SceneID, value *Scene) bool {
	l.mu.Lock()
	var found bool
...
func (l *SceneLoader) unsafeSet(key id.SceneID, value *Scene) {
	if l.cache == nil {
		l.cache = map[id.SceneID]*Scene{}
	}
	l.cache[key] = value
}
